...
**Current public version: `Horosa Skill 0.20.0` (78 callable tools).**
...
| Engineering tests | `284 / 284 pass` (ken / Shen Shu live integration + offline golden unit tests + node JS golden) |
...
| Release runtime | macOS (arm64) / Windows (x64) `v0.20.0` assets (ken + 14 Shen Shu engines bundled) packaged and verified; other platforms/arches fail install with an explicit unsupported error |
...
uv run pytest -q                                        # 284 passed with local backends up; live integration tests auto-skip offline
