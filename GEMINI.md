...
4. No `horosa` tools in the list? One command registers everything:
   `horosa-skill setup --client gemini` (`uv run horosa-skill setup --client gemini` inside this checkout),
...
5. **Compact surface.** By default this client sees only the 11 facade tools (`HOROSA_MCP_COMPACT=1`); every
   ...
   No `horosa` tools at all? `horosa-skill setup --client gemini` registers everything.
