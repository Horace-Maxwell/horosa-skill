...
**当前公开版本：`Horosa Skill 0.17.0`（77 个可调用工具）。**
...
| --- | --- |
| 可调用工具 | `77 / 77 ok=true` |
| 工程测试 | `274 / 274 pass`（含 ken / 神数 后端实时集成测试 + 离线 golden 单测 + node JS golden） |
...
| Release runtime | macOS (arm64) / Windows (x64) `v0.17.0` assets（含 ken + 14 神数引擎）已打包并校验；其余平台/架构安装时会明确报不支持 |
...
| `acg` ⓟ | 占星地图 | 行星地理投影线（MC/IC 经度、天顶点、偕升纬度带、线交点） |
| `astrodata` ⓟ | 名人星盘库 | 离线名人出生数据检索（FTS/分类/Rodden 评级，单人详情可直接排盘） |
...
| `heluo` ⓟ | 河洛理数 | 先天 / 后天卦与元堂、命运篇、大限·岁运 / 流年·岁运与断验（含元堂爻辞，本地 in-process） |
| `yizhangjing` ⓟ | 一掌经 | 十二支六道四柱四宫、命宫 / 人事十二宫、大限 / 小限流年十二神、神煞合参（本地 in-process） |
...
uv run pytest -q                                        # 274 passed（本机后端全起；纯离线时 live 集成测试自动 skip）
