...
- **A new release published as `latest` is repeatedly missing its Windows half — ALWAYS check the release
  ...
  **v0.17.0+ established a THIRD, stealthier mode — "pin-forward" (every release since):** the new release's
  manifest lists `win32-x64` but points it at a *previous* version's win zip. v0.17/v0.18/v0.19 pinned a
  baseline frozen at **v0.16.1**; from v0.20.0 the pin *advances* to the most recent real win zip you shipped
  (v0.20.0 pinned v0.19.0's zip), so staleness is now one-ish release rather than many — but it is still a
  pin, so **expect a `--check` GAP on every new release** and ship the real per-version half each time. **The guard stays GREEN and `install` does
