...
def test_time_basis_line_mirrors_upstream_builder() -> None:
    ...
    assert svc._build_time_basis_line(time_alg=0, late_zi_hour_use_next_day=1, after23_new_day=0) == (
        "时间基准：真太阳时(经度+均时差校正)；晚子时归次日：是；23 点换日：否"
    )
    # timeAlg 缺位 → 钟表时（七政 fieldsToParams 不带 timeAlg 的就是这一档）；开关缺位 → 否。
    assert svc._build_time_basis_line(time_alg=None, late_zi_hour_use_next_day=None, after23_new_day=None) == (
        "时间基准：钟表时(按输入钟面时刻,无真太阳时校正)；晚子时归次日：否；23 点换日：否"
    )
    assert svc._build_time_basis_line(time_alg=3, late_zi_hour_use_next_day="0", after23_new_day=True, zone="+08:00", note="补注") == (
        "时间基准：平太阳时(仅经度校正,无均时差)；晚子时归次日：否；23 点换日：是；时区：+08:00；补注"
    )
    # 未知档原样回显（JS `TIME_ALG_LABEL[x] || x`）。
    assert svc._build_time_basis_line(time_alg=7, late_zi_hour_use_next_day=1, after23_new_day=1).startswith("时间基准：7；")
