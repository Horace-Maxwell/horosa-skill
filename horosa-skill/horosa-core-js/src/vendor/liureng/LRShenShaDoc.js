...
export const SHENSHA_LIST = [
	...
	// 三合局标准神煞(补全 §19.1 十神之缺):将星=帝旺(仲)、灾煞=旺神之冲、岁煞=灾煞后一、
	// [Issue#75 同批] 攀鞍=将星后一(衰位)、六厄=驿马后一(死位)——旧注释写反(攀鞍=驿马后一/
	// 六厄=驿马前一),依错注释生成的两张表恰好整体对调 24 格。三合十二神煞标准序:
	// 劫煞(绝)灾煞(胎)天煞(养)指背(长生)咸池(沐浴)月煞(冠带)亡神(临官)将星(帝旺)攀鞍(衰)驿马(病)六厄(死)华盖(墓)。
	...
	{ name: '攀鞍', by: 'dayZhi', luck: 'good', brief: '进身、晋擢、佐助', rule: { '申': '丑', '子': '丑', '辰': '丑', '寅': '未', '午': '未', '戌': '未', '巳': '戌', '酉': '戌', '丑': '戌', '亥': '辰', '卯': '辰', '未': '辰' } },
	{ name: '六厄', by: 'dayZhi', luck: 'bad', brief: '险阻、留难、厄会', rule: { '申': '卯', '子': '卯', '辰': '卯', '寅': '酉', '午': '酉', '戌': '酉', '巳': '子', '酉': '子', '丑': '子', '亥': '午', '卯': '午', '未': '午' } },
];
