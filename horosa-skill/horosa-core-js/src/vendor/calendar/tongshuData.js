...
// [Q-423/T-387] 原「关键凶煞 KEY_XIONG_SHA」已删:全仓零消费(只有定义行),注释却自称吉日榜/日子馆在用;
// 表里 受死 / 四绝 / 岁破 三名在日历库词表中根本不存在(库内实名「致死」),留着只会误导后人照抄。
// 真正生效的硬排除/软扣分两表在 yearAuspicious.js(HARD_EXCLUDE / SOFT_XIONG),名字逐个对过库词表。
// 关键吉神（年度吉日榜加分用；名逐个对过日历库词表）。
// [Q-423/T-387] 删「天贵」:日历库吉神词表无此名(计数 0)→ 该项永不命中,是死值。
export const KEY_JI_SHEN = ['天德', '月德', '天德合', '月德合', '天愿', '天赦', '天喜', '三合', '六合', '天医', '福生'];
...
export const YONGSHI_YIJI_SYN = {
	...
	设醮: ['斋醮', '齐醮'],
	迁徙: ['移徙'],
	// [Q-271/ZC-18] 剃头=理发(同一事,日宜忌表实名「理发」;此前董公用事与择日「宜含 / 忌避」对该词恒无断)
	剃头: ['理发'],
};
