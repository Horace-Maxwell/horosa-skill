...
export function buildLiurengZeriSnapshotExtra({ cfg, geo, natal, tree, results, truncated }){
	const lines = [];
	lines.push('[择时搜索配置]');
	...
	if(rows.length){
		rows.slice(0, 60).forEach((r, i)=>{
			lines.push(`${i + 1}. ${r.start} ~ ${r.end}(${r.durationMin}分)${r.keText ? ` ${r.keText}` : ''}${r.chuanText ? `·${r.chuanText}` : ''}`);
		});
		if(rows.length > 60){
