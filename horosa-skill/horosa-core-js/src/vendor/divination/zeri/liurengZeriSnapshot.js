// [Z5·六壬择日] AI 快照附加段 builder。三段头🔒与 AI_EXPORT_PRESET_SECTIONS.liurengzeri
// 追加段逐字成对(preflight 对偶锁);基底=六壬全文快照(LiuRengMain 同链),此处只拼择时态。
import { liurengLeafSummary } from './liurengZeriConditionTypes.js';
...
function treeLines(node, depth, index, out){
	...
	if(node.kind === 'group' || Array.isArray(node.children)){
		...
		out.push(`${indent}${joiner}${liurengLeafSummary(node)}`);
	}
	return out;
}

export function buildLiurengZeriSnapshotExtra({ cfg, geo, natal, tree, results, truncated, explainAt, maxRows, explainRows }){
	const lines = [];
	lines.push('[择时搜索配置]');
	...
	if(rows.length){
		...
		appendZeriHitRows(lines, rows, {
			formatRow: (r, i)=>`${i + 1}. ${r.start} ~ ${r.end}(${r.durationMin}分)${r.keText ? ` ${r.keText}` : ''}${r.chuanText ? `·${r.chuanText}` : ''}`,
			tail: (total, cap)=>`…共 ${total} 段(仅列前 ${cap})`,
			truncated, truncatedText: '(扫描达上限截断,清单不完整)',
			maxRows, explainRows, explainAt, uiTree: tree, leafSummary: liurengLeafSummary,
		});
	}else{
		lines.push('(尚未择时或无命中)');
	}
