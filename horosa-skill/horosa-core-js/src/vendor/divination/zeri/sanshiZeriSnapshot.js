...
export function buildSanshiZeriSnapshotExtra({ cfg, geo, natal, tree, results, truncated, explainAt, maxRows, explainRows }){
	const lines = [];
	lines.push('[择时搜索配置]');
	...
	if(rows.length){
		...
		appendZeriHitRows(lines, rows, {
			formatRow: (r, i)=>`${i + 1}. ${r.start} ~ ${r.end}(${r.durationMin}分)${r.sanshiText ? ` ${r.sanshiText}` : ''}`,
			tail: (total, cap)=>`…共 ${total} 段(仅列前 ${cap})`,
			truncated, truncatedText: '(扫描达上限截断,清单不完整)',
			maxRows, explainRows, explainAt, uiTree: tree, leafSummary: sanshiLeafSummary,
		});
	}else{
		lines.push('(尚未择时或无命中)');
	}
