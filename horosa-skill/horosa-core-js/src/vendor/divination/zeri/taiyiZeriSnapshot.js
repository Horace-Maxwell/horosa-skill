...
export function buildTaiyiZeriSnapshotExtra({ cfg, geo, tree, results, truncated }){
	const lines = [];
	lines.push('[择时搜索配置]');
	...
	if(rows.length){
		rows.slice(0, 60).forEach((r, i)=>{
			lines.push(`${i + 1}. ${r.start} ~ ${r.end}(${r.durationMin}分)${r.juText ? ` ${r.juText}` : ''}${r.taiyiText ? `·${r.taiyiText}` : ''}`);
		});
		if(rows.length > 60){
