// [Z3·太乙择日] AI 快照附加段 builder。三段头🔒与 AI_EXPORT_PRESET_SECTIONS.taiyizeri
// 追加段逐字成对(preflight 对偶锁);基底=太乙全文快照(TaiYiMain 同链),此处只拼择时态。
import { taiyiLeafSummary } from './taiyiZeriConditionTypes.js';
...
function treeLines(node, depth, index, out){
	...
	if(node.kind === 'group' || Array.isArray(node.children)){
		...
		out.push(`${indent}${joiner}${taiyiLeafSummary(node)}`);
	}
	return out;
}

export function buildTaiyiZeriSnapshotExtra({ cfg, geo, tree, results, truncated, explainAt, maxRows, explainRows }){
	const lines = [];
	lines.push('[择时搜索配置]');
	...
	if(rows.length){
		...
		appendZeriHitRows(lines, rows, {
			formatRow: (r, i)=>`${i + 1}. ${r.start} ~ ${r.end}(${r.durationMin}分)${r.juText ? ` ${r.juText}` : ''}${r.taiyiText ? `·${r.taiyiText}` : ''}`,
			tail: (total, cap)=>`…共 ${total} 段(仅列前 ${cap})`,
			truncated, truncatedText: '(扫描达上限截断,清单不完整)',
			maxRows, explainRows, explainAt, uiTree: tree, leafSummary: taiyiLeafSummary,
		});
	}else{
		lines.push('(尚未择时或无命中)');
	}
