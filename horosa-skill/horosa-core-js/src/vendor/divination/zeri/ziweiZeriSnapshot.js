...
export function buildZiweiZeriSnapshotExtra({ cfg, geo, natal, tree, results, truncated }){
	const lines = [];
	lines.push('[择时搜索配置]');
	...
	if(rows.length){
		rows.slice(0, 60).forEach((r, i)=>{
			lines.push(`${i + 1}. ${r.start} ~ ${r.end}(${r.durationMin}分)${r.mingText ? ` ${r.mingText}` : ''}${r.juText ? `·${r.juText}` : ''}`);
		});
		if(rows.length > 60){
